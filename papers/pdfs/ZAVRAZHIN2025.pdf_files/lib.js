...
(function() {
    $(function() {
        ...
        var getDuration = function(grammar, layout, level, figures, figures_count, graphic, video) {
            var m = moment();
            var days = 0;
            ...
            if (0 !== days) {
                result = "Manuscript editing will be completed within " + days + " day(s) of payment";
                
                if (figures && figures_count > 20) {
                    result += " (submissions with more than 20 figures or tables may require one extra day for processing).<br/>";
                } else {
                    result += ".<br/>";
                }
            } 
            ...
            if (graphic) {
                result += "The Graphical Abstract first draft will be completed within 5 working days after payment and material submission.<br/>";
            }

            if (video) {
                result += "The final video will be delivered within 15 working days of payment, excluding correspondence or revision delays.<br/>";
            }

            if (extraNote) {
                result += "With Rapid, receive your work within " + (days - 4) + " day(s) of payment.<br/>";
            }

            result += "To add discounts to your order, click \"Submit\".";

            return result;
        }

        var set_price = function(word_num, has_manuscript, grammar, layout, level, rate, currency, article_type, figures, figures_num, graphic, video, video_type) {
            var data = {
                'num_words'          : word_num,
                ...
                'video_service'      : video ? 1 : 0,
                'video_type'         : video_type ? video_type : 1,
            };
            ...
            $.post('/english-editing', data).done(function (data) {
                ...
                var videoPrice = data['video_price_chf'];
                ...
                var layoutDiscountPercentage = data['layout_price_discount_percentage'];
                ...
                videoPrice *= rate;
                videoPrice = videoPrice.toFixed(2);
                ...
                $("#video-price").html(currency + " " + videoPrice);
                ...
                $("#layout-editing-discount-percentage").html("(" + layoutDiscountPercentage + "% discount)");

                $("#js-language-editing-level").html("");
                if ("" !== $("#level option:selected").text()) {
                    $("#js-language-editing-level").html(" - " + $("#level option:selected").text());
                }

                $("#js-video-type").html("");
                if ("" !== $("#video-type option:selected").text()) {
                    $("#js-video-type").html(" - " + $("#video-type option:selected").text());
                }
                ...
                $("#video-price-div").toggle(videoPrice > 0);
                ...
                $("#layout-editing-discount-percentage").toggle(layoutDiscountPercentage > 0);
        ...
        var getEncodedParamsString = function(word_num, grammar, layout, level, subject, currency, article_type, figures, figures_num, graphic, video, video_type) 
        {
            ...
            if (video && video_type) {
                params += "&video=1&video_type=" + video_type;
            }
        ...
        var updateSubmitLink = function(submit) 
        {
            ...
            var video         = $("#service-video").is(":checked");
            ...
            if (video) {
                video_type = $("select#video-type").val();
            }
            ...
            */
            ...
            if (!grammar && !figures && !layout && !graphic && !video) {
                $("#service-grammar").closest(".js-element").addClass("element error");
                valid = false;
            }

            if (grammar && (isNaN(word_num) || word_num <= 0)) {
            ...
            if (valid) {
                set_price(word_num, false, grammar, layout, level, rate, currency, article_type, figures, figures_count, graphic, video, video_type);
                $("#duration").html(getDuration(grammar, layout, level, figures, figures_count, graphic, video));
                var params = getEncodedParamsString(word_num, grammar, layout, level, subject, currency, article_type, figures, figures_count, graphic, video, video_type);
                $('#proceed_btn').data('href', susy_host + 'user/author_services/upload' + "?params=" + params);
